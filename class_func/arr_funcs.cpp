...
void arrToFile(const arr<string> &array, fstream &out) {
    ...
    out.write(reinterpret_cast<char*>(&size), sizeof(size)); //write capacity
    out.write(reinterpret_cast<char*>(&size), sizeof(size)); //write size of array
    ...
    while (true){
        ...
        if (ch == '@') { //check array flag
            ...
            if (varName == name && !varIsExist) { //right var is found
                ...
                var.push_back(value);
                if (var.get_size() - 1 != index && index != -1){ //move to right place if needed
                    string tmp;
